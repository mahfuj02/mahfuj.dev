...
export function SiteHeader() {
  ...
  const navItems = isHome ? homeNavItems : secondaryNavItems;
  const [visible, setVisible] = useState(false);
  const [mobileOpen, setMobileOpen] = useState(false);

  const desktopVisibilityClasses = isHome
    ? visible
  ...
  return (
    <header
      className={`fixed inset-x-0 top-0 z-50 border-b border-zinc-800/70 bg-zinc-950/65 backdrop-blur-xl transition-all duration-300 translate-y-0 opacity-100 ${desktopVisibilityClasses}`}
    >
      <nav className="mx-auto flex h-16 w-full max-w-6xl items-center justify-between px-6">
        <Link href="/" className="text-xs font-semibold tracking-[0.22em] text-zinc-100">
          MAHFUJ AHMED
        </Link>
        <button
          type="button"
